...
// 	GLubyte color_write_mask;
	...
	sbuf_type* stencilbuf_ptr(GLint x,GLint y)
	{
	...
	{
		device::draw(color_buffer_frontleft/*+BUFFER_INDEX_RGB(t,l)*/,l,t,w,h,buffer_line_bytes_rgba);
		//device::draw(depth_buffer,l,t,w,h);
	}
...
// 		void plot(GLint x,GLint y,zbuf_type z,cbuf_type* cbuf,zbuf_type* zbuf,cbuf_type* rgba)
		{
			...
			if(rgba&&test_alpha&&!(COMPARE_UINT(rgba[3],glctx.alpha_ref)&glctx.alpha_test_mask))
			{
				return;
			}
			...
			// if depth test not enabled, always pass? check specification		
			zbuf_type* zbuf=depth_buffer+BUFFER_INDEX_DEPTH(y,x);
			if(!test_depth||COMPARE_UINT(z,*zbuf)&glctx.depth_test_mask) // z-pass
			{
				sbuf_type dst=*sbuf;
